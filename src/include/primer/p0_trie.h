...
class Trie {
 ...
 public:
  ...
  bool Insert(const std::string &key, T value) {
    ...
    for (int i = 0; i < static_cast<int>(key.size()); i++) {
      if (i == static_cast<int>(key.size() - 1)) {
        if ((child = current->get()->GetChildNode(key[i])) == nullptr) {
          current->get()->InsertChildNode(key[i], std::make_unique<TrieNodeWithValue<T>>(key[i], value));
        } else {
        ...
        if ((child = current->get()->GetChildNode(key[i])) == nullptr) {
          child = current->get()->InsertChildNode(key[i], std::make_unique<TrieNode>(key[i]));
        }
        current = child;
      }
    }
  ...
  bool Remove(const std::string &key) {
    latch_.WLock();
    bool r = RmImpl(key, &root_);
    latch_.WUnlock();
    return r;
  }

  bool RmImpl(const std::string &key, std::unique_ptr<TrieNode> *cur) {
    if (key.empty()) {
      return false;
    }
    std::unique_ptr<TrieNode> *child;
    child = cur->get()->GetChildNode(key[0]);
    if (child == nullptr) {
      return false;
    }
    if (key.size() == 1) {
      if (!child->get()->HasChildren()) {
        cur->get()->RemoveChildNode(key[0]);
      } else {
        *child = std::make_unique<TrieNode>(std::move(*(*child)));
      }
      return true;
    }
    std::string sub = key.substr(1, key.size());
    bool re = RmImpl(sub, child);
    if (!re) {
      return false;
    }
    if (!child->get()->HasChildren()) {
      cur->get()->RemoveChildNode(child->get()->GetKeyChar());
    }
    return true;
  }
  ...
  T GetValue(const std::string &key, bool *success) {
    ...
    *success = false;
    ...
    std::unique_ptr<TrieNode> *child;
    for (char i: key) {
      child = current->get()->GetChildNode(i);
      if (child == nullptr) {
        latch_.RUnlock();
        return {};
      }
    ...
    auto *c2 = dynamic_cast<TrieNodeWithValue<T> *>(current->get());
    if (c2 != nullptr) {
      *success = true;
      latch_.RUnlock();
      return c2->GetValue();
